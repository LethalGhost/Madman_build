...
/datum/surgery_step/generic/cauterize
	allowed_tools = list(
	/obj/item/weapon/sutury = 100,			\
	/obj/item/weapon/wire/ = 75
	)
...
/datum/surgery_step/generic/cauterize/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("[user] is beginning to sew up the incision on [target]'s [affected.name] with \the [tool]." , \
	"You are beginning to sew up the incision on [target]'s [affected.name] with \the [tool].")
...
/datum/surgery_step/generic/cauterize/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='notice'>[user] sews up the incision on [target]'s [affected.name] with \the [tool].</span>", \
	"<span class='notice'>You sew up the incision on [target]'s [affected.name] with \the [tool].</span>")
	affected.open = 0
...
/datum/surgery_step/generic/cauterize/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='warning'>[user]'s hand slips, leaving a small scratch on [target]'s [affected.name] with \the [tool]!</span>", \
	"<span class='warning'>Your hand slips, leaving a small scratch on [target]'s [affected.name] with \the [tool]!</span>")
